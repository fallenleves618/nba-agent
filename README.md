...
- 仓库里的 `config/*.json` 只保留 `${ENV_VAR}` 占位符，不要提交真实 `webhook_url`、`secret`、`api_key`
- 真实凭证统一放到 shell 环境变量里，本地 `source ~/.zshrc` 后再运行
- 提交前至少执行一次 `rg -n "hook/|sk-|AKIA|AIza|api_key|secret" config README.md nba_agent`
